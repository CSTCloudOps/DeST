...
def RCL(time_model,space_model, test_samples, cases, node_dict, split_ratio=0.3, method='num', t_value=3, before=60, after=300, verbose=False):
    spilit_index = int(len(cases)*split_ratio)
    instance_representations = aggregate_instance_representations(cases[spilit_index:], ILD(time_model,space_model, test_samples), before, after)
    failure_representations, type_labels = aggregate_failure_representations(cases[spilit_index:], SLD(time_model,space_model, test_samples, method, t_value), None, before, after)
